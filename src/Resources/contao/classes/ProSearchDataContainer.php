...
class ProSearchDataContainer extends DataContainer
{
    ...
    public function createButtons($arrRow)
    {
        ...
        if ((isset($operations['editheader']) && $operations['editheader']) || (isset($operations['edit']) && $operations['edit'])) {
            ...
            $return .= '<div class="title"><span class="icon" title="' . strtoupper(
                    $arrRow['shortcut']
                ) . '">' . $arrRow['icon'] . '</span><a href="' . $this->addToSearchUrl(
                    $arrRow,
                    $queryStr
                ) . '&popup=1" class="search-result" tabindex="1" onclick="Backend.openModalIframe({\'width\':960,\'title\':\'' . $arrRow['title'] . '\',\'url\':this.href});return false"><span>' . mb_convert_encoding(
                    $title,
                    'UTF-8'
                ) . '</span> <span class="info">' . $tagsStr . '</span></a></div>';
        }
        ...
        if (is_array($ctableArr) && !empty($ctableArr) && $mode != 5 && $fmTable != 'fm') {
            ...
            $return .= '<a href="' . $this->addToSearchUrl($arrRow, $queryStr) . '" tabindex="1">' . Image::getHtml(
                    $icon,
                    $arrRow['title']
                ) . '</a>';
        }
        ...
        if ($arrRow['doTable'] == 'page') {
            ...
            $icon = $arrRow['icon'];
            $return .= '<a href="' . $this->addToSearchUrl(
                    $arrRow,
                    $pageNodeQueryStr
                ) . '" tabindex="1">' . $icon . '</a>';
            ...
            $return .= '<a href="' . $this->addToSearchUrl(
                    $arrRow,
                    $articleNodeQueryStr
                ) . '" tabindex="1">' . Image::getHtml($icon, $arrRow['title']) . '</a>';
            ...
            if ($arrRow['type'] == 'regular') {
                ...
                $return .= '<a href="' . $this->addToSearchUrl(
                        $arrRow,
                        $feQueryStr
                    ) . '" target="_blank" tabindex="1">' . Image::getHtml($icon, $arrRow['title']) . '</a>';
            }
        }
        ...
        if ($operations['show'] && $arrRow['dca'] != 'tl_files') {
            ...
            $attributes = (array_key_exists(
                    'attributes',
                    $operations['show']
                ) && $operations['show']['attributes'] != '') ? ' ' . ltrim(
                    sprintf($operations['show']['attributes'], $id, $id)
                ) : '';
            $queryStr = $href . $id . $table . '&amp;popup=1';
            $return .= '<a href="' . $this->addToSearchUrl(
                    $arrRow,
                    $queryStr
                ) . '" tabindex="1" onclick="Backend.openModalIframe({\'width\':768,\'title\':\'' . specialchars(
                    str_replace("'", "\\'", sprintf($arrRow['title'], $arrRow['docId']))
                ) . '\',\'url\':this.href});return false"' . $attributes . '>' . Image::getHtml($icon) . '</a> ';
        }
        ...
        if ($operations['show'] && $arrRow['dca'] == 'tl_files') {
            ...
            $return .= '<a href="' . $href . '" tabindex="1" onclick="Backend.openModalIframe({\'width\':768,\'title\':\'' . str_replace(
                    "'",
                    "\\'",
                    specialchars($arrRow['title'], false, true)
                ) . '\',\'url\':this.href,\'height\':500});return false" >' . Image::getHtml($icon) . '</a>';
        }
